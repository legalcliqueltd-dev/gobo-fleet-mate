...
export default function Signup() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (error) setErrorMsg(friendlyAuthError(error, 'signup'));
  };
    ...
    <div className="flex items-center justify-center py-8 md:py-16">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground mt-1.5">Start tracking your fleet in minutes</p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 md:p-8 space-y-5">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {infoMsg && (
              <div className="rounded-lg bg-success/10 border border-success/20 px-4 py-3">
                <p className="text-sm text-success">{infoMsg}</p>
              </div>
            )}
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link to="/auth/login" className="text-primary font-medium hover:text-primary/80 transition-colors">
              Log in
            </Link>
          </p>
        </div>
